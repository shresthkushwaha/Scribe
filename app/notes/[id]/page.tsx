...
export default function NoteEditorPage() {
    ...
    const allStoreTags = useMemo(() => {
        ...
        notes.forEach((n: Note) => n.tags?.forEach(t => set.add(t)));
    ...
    const displaySuggestions = useMemo(() => {
        let suggestions = [...autoTags];
        if (tagInput.trim()) {
            const match = tagInput.trim().toLowerCase();
            const matching = allStoreTags.filter(t => t.includes(match) && !tags.includes(t));
            suggestions = [...matching, ...suggestions];
        } else {
            const recent = allStoreTags.filter(t => !tags.includes(t)).slice(0, 5);
            suggestions = [...recent, ...suggestions];
        }
        return Array.from(new Set(suggestions)).slice(0, 8);
    }, [autoTags, tagInput, allStoreTags, tags]);
                                        ...
                                        <>
                                            ...
                                            <div className="absolute right-0 top-full mt-2 w-56 bg-white border border-[var(--border-soft)] rounded-[var(--radius-lg)] shadow-[0_12px_24px_-10px_rgba(62,56,56,0.15)] p-2 z-50 flex flex-col gap-1 animate-in fade-in slide-in-from-top-2">
                                                ...
                                                <button onClick={() => { handleDownload(); setShowMobileActions(false); }}
                                                    className="flex items-center gap-3 px-4 py-3 rounded-[var(--radius-md)] text-[14px] font-medium hover:bg-[var(--bg-muted)] transition-colors">
                                                    <DownloadSimple size={18} weight="bold" className="text-[var(--ink-dim)]" />
                                                    <span style={{ color: 'var(--ink)' }}>Export to .md</span>
                                                </button>
                        ...
                        {displaySuggestions.length > 0 && (
                            <div className="mt-6">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {displaySuggestions.map((t: string) => (
